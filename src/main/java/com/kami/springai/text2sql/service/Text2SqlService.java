...
public class Text2SqlService {
        ...
        try {
            ...
            if (sql == null || !validationResult.isOverallValid()) {
                String errorMsg = validationResult != null ? 
                    validationResult.getOverallMessage() : "无法生成有效的SQL语句";
    ...
    private String applyPatternToGenerate(GeneralizedSqlPattern pattern, QuerySemantic semantic, DatabaseSchema schema) {
        // 简化的模式应用，实际应用中会更复杂
    ...
    private void validateGeneratedSql(String sql) {
        ...
        // 额外检查：确保不是以其他SQL语句开头
        String[] forbiddenStarts = {"WITH", "SHOW", "DESCRIBE", "EXPLAIN"};
        for (String start : forbiddenStarts) {
            if (upperSql.startsWith(start)) {
                throw new RuntimeException("不允许的SQL类型: " + start);
            }
        }
